...
class FlightPriceListCreateAPIView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, flight_id):
        ...
        prices = list_service_prices(PriceHistory.ServiceType.FLIGHT, flight.id)
    ...
    def post(self, request, flight_id):
        ...
        if not serializer.is_valid():
            return Response(
                {'error': 'Datos invalidos para actualizar el precio del vuelo.', 'details': serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )

        new_price = set_service_price(
            PriceHistory.ServiceType.FLIGHT,
            flight.id,
