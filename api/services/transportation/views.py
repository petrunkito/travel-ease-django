...
from api.services.pricing import list_service_prices, set_service_price
from api.services.serializers import PriceHistorySerializer, ServicePriceUpdateSerializer
...
class TransportationPriceListCreateAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get_transportation(self, transportation_id):
        try:
            return Transportation.objects.get(pk=transportation_id)
        except Transportation.DoesNotExist:
            return None

    def get(self, request, transportation_id):
        transportation = self.get_transportation(transportation_id)
        if not transportation:
            return Response({'error': 'Transporte no encontrado.'}, status=status.HTTP_404_NOT_FOUND)

        prices = list_service_prices(PriceHistory.ServiceType.TRANSPORTATION, transportation.id)
        serializer = PriceHistorySerializer(prices, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def post(self, request, transportation_id):
        transportation = self.get_transportation(transportation_id)
        if not transportation:
            return Response({'error': 'Transporte no encontrado.'}, status=status.HTTP_404_NOT_FOUND)

        serializer = ServicePriceUpdateSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(
                {
                    'error': 'Datos invalidos para actualizar el precio del transporte.',
                    'details': serializer.errors,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )

        new_price = set_service_price(
            PriceHistory.ServiceType.TRANSPORTATION,
            transportation.id,
            serializer.validated_data['price'],
        )
        output = PriceHistorySerializer(new_price)
        return Response(output.data, status=status.HTTP_201_CREATED)
