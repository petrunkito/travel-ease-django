...
class HotelPriceListCreateAPIView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, hotel_id):
        ...
        prices = list_service_prices(PriceHistory.ServiceType.HOTEL, hotel.id)
    ...
    def post(self, request, hotel_id):
        ...
        if not serializer.is_valid():
            return Response(
                {'error': 'Datos invalidos para actualizar el precio del hotel.', 'details': serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )

        new_price = set_service_price(
            PriceHistory.ServiceType.HOTEL,
            hotel.id,
